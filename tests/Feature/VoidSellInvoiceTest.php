...
class VoidSellInvoiceTest extends TestCase
{
    ...
    public function test_recalculating_product_stock_updates_invoice_item_quantity_snapshots(): void
    {
        $product = $this->createProduct();

        $buy = $this->buy([$this->productItem($product, 10, 100)], true, 7027, '2026-07-01')['invoice'];
        $sell = $this->sell([$this->productItem($product, 4, 180)], true, 7028, '2026-07-02')['invoice'];
        $returnSell = $this->returnSell([$this->productItem($product, 2, 180)], $sell->id, true, 7029, '2026-07-03')['invoice'];

        foreach ([$buy, $sell, $returnSell] as $invoice) {
            $this->findInvoiceItem($invoice, $product)->update(['quantity_at' => 999]);
        }
        $product->update(['quantity' => 999]);
        ...
        $this->assertSame(0.0, (float) $this->findInvoiceItem($buy, $product)->quantity_at);
        $this->assertSame(10.0, (float) $this->findInvoiceItem($sell, $product)->quantity_at);
        $this->assertSame(6.0, (float) $this->findInvoiceItem($returnSell, $product)->quantity_at);
