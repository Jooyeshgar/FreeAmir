...
class AncillaryCostTransactionBuilder
{
    ...
    private function buildAncillaryCostTransaction(): void
    {
        foreach ($this->ancillaryCost as $item) {
            ...
            $this->transactions[] = [
                ...
                'desc' => __('Ancillary Cost for :item', ['item' => $product->name]).' '.__('On Invoice').' '.$this->invoiceType.' '.formatDocumentNumber($this->invoice->number),
                'value' => -$item['amount'],
            ];
        }
    }
    ...
    private function buildVatTransaction(): void
    {
        if ($this->data['vatPrice'] > 0) {
            $this->transactions[] = [
                ...
                'desc' => __('VAT Ancillary Cost').' '.__('Invoice').' '.$this->invoiceType.' '.formatDocumentNumber($this->invoice->number),
    ...
    private function buildCustomerTransaction(): void
    {
        ...
        $this->transactions[] = [
            ...
            'desc' => __('Invoice').' '.$this->invoiceType.' '.__(' with number ').' '.formatDocumentNumber($this->invoice->number),
