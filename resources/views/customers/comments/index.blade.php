        ...
        <div class="card-body">
            <div class="card-actions flex justify-between gap-4 ">
                <a href="{{ route('comments.create', $customer->id) }}"
                    class="btn btn-primary ">{{ __('Create Comment') }}</a>
            </div>
                    ...
                    @foreach ($comments as $comment)
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                ...
                                <span class="text-xs text-gray-500 ml-1">{{ convertToFarsi($comment->rating) }}</span>
                            </td>
                            <td class="px-4 py-2">
                                <a href="{{ route('comments.index', $comment->id) }}"
                                    class="btn btn-sm btn-info">{{ __('Comment') }}</a>
