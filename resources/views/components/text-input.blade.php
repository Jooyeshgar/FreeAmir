<label {{ $attributes->merge(['class' => 'flex flex-col flex-wrap '.$attributes->get('label_class')]) }}>
    ...
    <input onkeyup="{{$attributes->get('onkeyup_input')}}" {{$attributes->get('disabled')?'disabled':''}}" id="{{ $attributes->get('id_input') }}" name="{{ $attributes->get('input_name') }}" value="{{ $attributes->get('input_value')}}"
