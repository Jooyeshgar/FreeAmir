@props(['belowMenu' => false])

@php($impersonateManager = app(\Lab404\Impersonate\Services\ImpersonateManager::class))

@if ($impersonateManager->isImpersonating())
    <aside @class([
        'sticky z-20 w-full border-b border-amber-300 bg-amber-100 text-amber-950 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100',
        'top-14' => $belowMenu,
        'top-0' => ! $belowMenu,
    ]) role="status">
