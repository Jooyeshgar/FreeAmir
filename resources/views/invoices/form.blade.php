                ...
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3"
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        <x-select-box url="{{ route('invoices.search-product-service') }}" :options="$options"
                            ...
                            transaction.item_type = selectedType;
                            transaction.item_id = selectedId ? (selectedType + '-' + selectedId) : '';"
                        ...
                        <x-text-input
                            x-bind:value="(transaction.total = (Number($store.utils.convertToEnglish(transaction.quantity)) || 0) *
                                ...
                                (((Number($store.utils.convertToEnglish(transaction.quantity)) || 0) *
                                        (Number($store.utils.convertToEnglish(transaction.unit)) || 0) - (Number($store
                                            .utils.convertToEnglish(transaction.off)) || 0)) *
...
<div class="mt-4 flex gap-2 justify-end">
    <a href="{{ route('invoices.index', ['invoice_type' => $invoice->invoice_type ?? $invoice_type]) }}"
        type="submit" class="btn btn-default rounded-md"> {{ __('cancel') }}
    </a>
