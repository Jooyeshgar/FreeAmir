            ...
            <template x-for="(transaction, index) in transactions" :key="transaction.id">
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3" @click="activeTab = index">
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        <label class="sr-only">{{ __('Services') }}</label>

                        @php
                            $hint = '<a class="link text-blue-500 dark:text-sky-300" href="' . route('services.create') . '">' . __('Create Service') . '</a>';
                        @endphp

                        <x-select-box url="{{ route('invoices.search-product-service') }}" :options="[['headerGroup' => 'service', 'options' => $services]]"
                            x-model="selectedValue" x-init="selectedValue = initItemSelection(transaction)" placeholder="{{ __('Select Service') }}"
                    ...
                    <div class="flex-1 w-[200px]">
                        <x-text-input x-bind:value="transaction.desc" placeholder="{{ __('description') }}" x-bind:name="'transactions[' + index + '][desc]'"
                            label_text_class="text-gray-500" label_class="w-full" input_class="border-white" x-bind:disabled="!transaction.service_id"></x-text-input>
                    </div>
...
<div class="mt-4 flex gap-2 justify-end">
    <a href="{{ route('invoices.index', ['invoice_type' => $invoice->invoice_type ?? $invoice_type]) }}" type="submit" class="btn btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600">{{ __('cancel') }}</a>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('transactionForm', () => ({
                ...
                initItemSelection(transaction) {
                    const id = transaction.service_id ?? null;
                    
                    transaction.item_type = 'service';
                    transaction.item_id = id ? `service-${id}` : '';
                    return id ? `service-${id}` : '';
                },
                selectItem(transaction, type, id) {
                    transaction.service_id = id;
                    transaction.item_type = 'service';
                    transaction.item_id = `service-${id}`;
                    ...
                    if (isEditable) {
                        ...
                        transaction.off = 0;
                        const vatRate = this.getServiceVat(id);
                        transaction.vat = (this.isReturnType || this.isEditing) ? this.calcVatValue(
                            vatRate, transaction
