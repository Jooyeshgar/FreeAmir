<x-card class="rounded-2xl w-full" class_body="p-4">
    <div class="flex gap-2 items-center justify-start">
        ...
        <input type="hidden" id="invoice_type" name="invoice_type" value="sell">
                ...
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3" @click="activeTab = index">
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        <x-select-box url="{{ route('invoices.search-product-service') }}" :options="$options"
                            x-model="selectedValue" x-init="selectedValue = initItemSelection(transaction)" placeholder="{{ __('Select Product/Service') }}"
                    ...
                    <div class="flex-1 w-[200px]">
                        <x-text-input x-bind:value="transaction.desc" placeholder="{{ __('description') }}" 
                            x-bind:name="'transactions[' + index + '][desc]'" label_text_class="text-gray-500" label_class="w-full" input_class="border-white"
                    ...
                    <div class="flex-1 min-w-24 max-w-32">
                        <x-text-input placeholder="0" x-model.number="transaction.quantity" x-bind:name="'transactions[' + index + '][quantity]'"
                            x-bind:disabled="(!transaction.product_id && !transaction.service_id) || transaction.service_id" label_text_class="text-gray-500" label_class="w-full" input_class="border-white">
                        </x-text-input>
                    </div>
                    <div class="flex-1 min-w-24 max-w-32">
                    ...
                    <div class="flex-1 min-w-32 max-w-32">
                        <x-text-input x-bind:value="calcTotal(transaction)" x-bind:name="'transactions[' + index + '][total]'"
                            placeholder="0" label_text_class="text-gray-500" label_class="w-full" input_class="border-white" readonly>
                        </x-text-input>
                    </div>
                </div>
            </template>
...
<div class="mt-4 flex gap-2 justify-end">
    ...
    @can('invoices.approve')
        <button id="submitFormAndApprove" type="submit" name="approve" value="1" class="btn text-white btn-primary rounded-md">{{ __('save and ready to approve') }}</button>
    @endcan
</div>
