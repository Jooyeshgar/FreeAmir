                ...
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3" @click="activeTab = index">
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        <x-select-box url="{{ route('invoices.search-product-service') }}" :options="[['headerGroup' => 'product', 'options' => $products]]"
                            ...
                            @selected="selectItem(transaction, $event.detail.type, $event.detail.id)" hint='{!! $hint !!}' />
                    ...
                    <div class="flex-1 w-[200px]">
                        <x-text-input x-bind:value="transaction.desc" placeholder="{{ __('description') }}" x-bind:name="'transactions[' + index + '][desc]'"
                            label_text_class="text-gray-500" label_class="w-full" input_class="border-white" x-bind:disabled="!transaction.product_id">
                        </x-text-input>
                    </div>
                    <div class="flex-1 min-w-24 max-w-32">
                        <x-text-input placeholder="0" x-model.number="transaction.quantity" x-bind:name="'transactions[' + index + '][quantity]'"
                            x-bind:disabled="!transaction.product_id" label_text_class="text-gray-500" label_class="w-full" input_class="border-white">
                        </x-text-input>
                    </div>
                    <div class="flex-1 min-w-24 max-w-32">
                        <x-text-input placeholder="0" x-model.number="transaction.off" x-bind:name="'transactions[' + index + '][off]'" 
                            x-bind:disabled="!transaction.product_id" label_text_class="text-gray-500" label_class="w-full" 
                            input_class="border-white" x-on:input="transaction.off = $store.utils.convertToEnglish($event.target.value); $event.target.value = $store.utils.formatNumber(transaction.off)">
                        </x-text-input>
                    </div>
    ...
    <div class="flex flex-row justify-between" x-data="{ subtractionsInput: '{{ old('subtraction') ?? ($invoice->subtraction ?? 0) }}' }">
        <div class="flex justify-start px-4 gap-4 py-3 rounded-b-2xl">
            <x-text-input placeholder="0" label_text_class="text-gray-500" label_class="w-full" input_name="subtraction" title="{{ __('Subtractions') }}"
                ...
                x-model="subtractionsInput" @input="$event.target.value = $store.utils.formatNumber($event.target.value)">
            </x-text-input>
        </div>
...
<div class="mt-4 flex gap-2 justify-end">
    <a href="{{ route('invoices.index', ['invoice_type' => 'buy']) }}" type="submit" class="btn btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600">{{ __('cancel') }}</a>
            ...
            Alpine.data('transactionForm', () => ({
                ...
                selectItem(transaction, type, id) {
                    ...
                    if (isEditable) {
                        ...
                        transaction.off = 0;
                        const vatRate = this.getProductVat(id);
