    ...
    <div class="flex gap-2 items-center justify-start">
        ...
        <div class="flex w-1/3">
            <div class="flex-wrap w-full" x-data="{
                ...
                <x-select-box url="{{ route('invoices.search', ['invoice_type' => 'return_buy']) }}" :options="[['headerGroup' => 'invoice', 'options' => $returnedInvoiceOptions]]"
            ...
            <template x-for="(transaction, index) in transactions" :key="transaction.id">
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3"
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        <select class="select select-bordered select-sm w-full mt-1" x-bind:name="'transactions[' + index + '][warehouse_id]'" x-model="transaction.warehouse_id" x-bind:disabled="!transaction.product_id || !lastYearsInvoices"><option value="">{{ __('Select Warehouse') }}</option>@foreach ($warehouses as $warehouse)<option value="{{ $warehouse->id }}">{{ $warehouse->name }}</option>@endforeach</select>
                    </div>
