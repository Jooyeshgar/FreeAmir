<x-card class="rounded-2xl w-full" class_body="p-4">
    <div class="flex gap-2 items-center justify-start">
        @php
            $initialReturnedInvoiceId = old('returned_invoice_id', $invoice->returned_invoice_id ?? ($prefilledReturnedInvoiceId ?? null));
        ...
        <div class="flex w-1/3">
            <div class="flex-wrap w-full" x-data="{
                ...
                <x-select-box url="{{ route('invoices.search', ['invoice_type' => 'return_buy']) }}" :options="[['headerGroup' => 'invoice', 'options' => $returnInvoices]]" x-model="selectedValue"
        ...
        <input type="hidden" id="invoice_type" name="invoice_type" value="return_buy">
        <div class="flex w-1/3">
            <x-text-input input_name="title" title="{{ __('Invoice Name') }}" input_value="{{ old('title') ?? ($invoice->title ?? '') }}" placeholder="{{ __('Invoice Name') }}"
                label_text_class="text-gray-500" label_class="w-1/2"></x-text-input>
        </div>

        <div class="flex w-1/4">
            @php
                $matchedReturnedInvoice = $initialReturnedInvoiceId ? collect($returnInvoices)->firstWhere('id', (int) $initialReturnedInvoiceId) : null;
                ...
                <template x-if="customerSelectRender">
                    <x-select-box url="{{ route('invoices.search-customer') }}" :options="[['headerGroup' => 'customer', 'options' => $customers]]" x-model="selectedValue"
                        x-init="if (!selectedValue && customer_id) {
                            selectedValue = 'customer-' + customer_id;
                        }" placeholder="{{ __('Select Customer') }}" hint='{!! $hint !!}' @selected="customer_id = $event.detail.id;" :disabled="true" />
                </template>
    ...
    <div class="flex justify-start gap-2 mt-2">
        ...
        <x-text-input input_value="{{ old('invoice_number') ?? formatDocumentNumber($invoice->number ?? $previousInvoiceNumber + 1) }}"
            input_name="invoice_number" title="{{ __('Current Invoice Number') }}" placeholder="{{ __('Current Invoice Number') }}"
        ...
        <x-text-input data-jdp title="{{ __('date') }}" input_name="date" placeholder="{{ __('date') }}"
            input_value="{{ old('date') ?? convertToJalali($invoice->date ?? now()) }}" label_text_class="text-gray-500 text-nowrap" input_class="datePicker"></x-text-input>
    </div>
</x-card>
            ...
            <div class="text-sm flex-1 min-w-24 max-w-64 text-center text-gray-500 pt-3 flex items-center justify-center gap-2">
                <div class="flex items-center gap-3 ml-1">
                    @if (!$isReturnServiceBuy)
                        {{ __('Product') }}
                        <a href="{{ route('products.create') }}" class="flex items-center gap-1 btn btn-xs btn-ghost text-blue-500 hover:text-blue-700 dark:text-sky-300 dark:hover:text-sky-200 dark:hover:bg-sky-500/10"
                    ...
                    @else
                        {{ __('Services') }}
                        <a href="{{ route('services.create') }}" class="flex items-center gap-1 btn btn-xs btn-ghost text-blue-500 hover:text-blue-700 dark:text-sky-300 dark:hover:text-sky-200 dark:hover:bg-sky-500/10"
                ...
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3" @click="activeTab = index">
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        @php
                            ...
                            $hint = !$isReturnServiceBuy ? '<a class="link text-blue-500 dark:text-sky-300" href="' . route('products.create') . '">' . __('Create Product') . '</a>'
                                    : '<a class="link text-blue-500 dark:text-sky-300" href="' . route('services.create') . '">' . __('Create Service') . '</a>';
                            $hint2 = $isReturnServiceBuy ? '<a class="link text-blue-500 dark:text-sky-300" href="' . route('services.create') . '">' . __('Create Service') . '</a>' : '';
                        @endphp
                    ...
                    <div class="flex-1 min-w-24 max-w-32">
                        <x-text-input placeholder="0" x-model.number="transaction.off" x-bind:name="'transactions[' + index + '][off]'"
                            x-bind:disabled="!transaction.product_id && !transaction.service_id" x-bind:readonly="true"
                            x-bind:class="'bg-base-200 opacity-60 cursor-not-allowed'" label_text_class="text-gray-500" label_class="w-full" input_class="border-white"
...
<div class="mt-4 flex gap-2 justify-end">
    <a href="{{ route('invoices.index', ['invoice_type' => 'return_buy']) }}" type="submit" class="btn btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600">{{ __('cancel') }}</a>
    <button id="submitForm" type="submit" class="btn text-white btn-primary rounded-md">{{ __('save') }} </button>

    @can('invoices.approve')
        <button id="submitFormAndApprove" type="submit" name="approve" value="1" class="btn text-white btn-primary rounded-md">{{ __('save and approve') }}</button>
    @endcan
</div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('transactionForm', () => ({
                ...
                selectedReturnInvoiceId: {!! json_encode(old('returned_invoice_id', $invoice->returned_invoice_id ?? ($prefilledReturnedInvoiceId ?? null)), JSON_UNESCAPED_UNICODE) !!},
                init() {
                ...
                selectItem(transaction, type, id) {
                    ...
                    const isEditable = !this.isEditing || transaction.unit == null || transaction.vat == null;
                    if (isEditable) {
                ...
                calcTotal(t) {
                    ...
                    t.total = vatIsValue ? subtotal - off + vat : subtotal + ((subtotal - off) * vat / 100) - off;
