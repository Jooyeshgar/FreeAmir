    ...
    <div class="flex gap-2 items-center justify-start">
        ...
        <div class="flex w-1/3">
            <div class="flex-wrap w-full" x-data="{
                ...
                <x-select-box url="{{ route('invoices.search', ['invoice_type' => 'return_sell']) }}" :options="[['headerGroup' => 'invoice', 'options' => $returnInvoices]]" x-model="selectedValue"
        ...
        <div class="flex w-1/3">
            <x-text-input input_name="title" title="{{ __('Invoice Name') }}" input_value="{{ old('title') ?? ($invoice->title ?? '') }}"
                ...
                <div :class="{ 'active': activeTab === index }" class="transaction flex gap-2 items-center px-4 pb-3" @click="activeTab = index">
                    ...
                    <div class="flex-1 min-w-24 max-w-64">
                        ...
                        <x-select-box url="{{ route('invoices.search-product-service') }}" :options="$options" x-model="selectedValue"
                            x-init="selectedValue = initItemSelection(transaction)" placeholder="{{ __('Select Product/Service') }}"
                            @selected="selectItem(transaction, $event.detail.type, $event.detail.id)" hint='{!! $hint !!}' hint2='{!! $hint2 !!}' :disabled="true" />
...
<div class="mt-4 flex gap-2 justify-end">
    <a href="{{ route('invoices.index', ['invoice_type' => 'return_sell']) }}" type="submit" class="btn btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600">{{ __('cancel') }}</a>
