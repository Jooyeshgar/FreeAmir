            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="stats shadow bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200/60">
                    <div class="stat">
                        ...
                        <div class="stat-value text-amber-600 text-3xl">{{ formatNumber($invoice->items->reduce(fn($carry, $item) => $carry + ($item->unit_discount ?? 0), 0)) }}</div>
                ...
                <div class="stats shadow bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200/60">
                    <div class="stat">
                        ...
                        <div class="stat-value text-emerald-600 text-3xl">{{ formatNumber($invoice->items->reduce(fn($carry, $item) => $carry + ($item->vat ?? 0), 0)) }}</div>
                    ...
                    @can('invoices.approve')
                        @if ($changeStatusValidation->hasErrors())
                            <span data-tip="{{ $changeStatusValidation->toText() ?? '' }}" class="tooltip">
                                <button class="btn btn-sm {{ $invoice->status->isApproved() ? 'btn-warning' : 'btn-success' }} {{ $changeStatusValidation->hasWarning() ? ' btn-outline ' : '' }} btn-disabled cursor-not-allowed"
                        ...
                        @else
                            <a x-data="{}"
                                ...
                                data-tip="{{ $changeStatusValidation->toText() ?? '' }}" href="{{ route('invoices.change-status', [$invoice, $invoice->status->isApproved() ? 'unapproved' : 'approved']) }}"
