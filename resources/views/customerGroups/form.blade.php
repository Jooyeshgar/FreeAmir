        ...
        <div class="transaction flex gap-2 items-center px-4 ">
            ...
            <div class="flex-1 min-w-24 max-w-64 pb-3">
                ...
                <x-text-input disabled
                    input_value="{{ !empty($customerGroup->subject) ? $customerGroup->subject->formattedCode() : '' }}"
                    id="value" label_text_class="text-gray-500" label_class="w-full"
                    input_class="border-white value codeInput "></x-text-input>

            </div>
            <input type="text" class="subject_id" name="subject_id" hidden value="">
            <x-subject-select-box :subjects="$subjects" input_value="{{ $customerGroup->subject->name ?? '' }}"
                placeholder="{{ __('Select a subject') }}" allSelectable="true"></x-subject-select-box>
