    ...
    <div class="mb-6 px-2 flex justify-between gap-4 mt-2">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                {{ __('Manage every company record and its fiscal-year access.') }}</p>
        </div>
        @if ($canCreateFirstCompany)
            <a href="{{ route('companies.create') }}" data-testid="create-first-company"
                class="btn btn-primary gap-2 rounded-xl shadow-lg shadow-primary/15">
    ...
    <section
        ...
        <div
            ...
            <form action="{{ route('companies.index') }}" method="GET"
                ...
                <select name="status"
                    class="select select-bordered h-9 min-h-9 w-36 shrink-0 rounded-lg bg-slate-50 text-sm dark:bg-slate-950/50"
                ...
                <button class="btn btn-neutral btn-sm h-9 min-h-9 shrink-0 rounded-lg px-3"
                        ...
                        <tr
                            ...
                            <td>
                                <div class="flex min-w-48 items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-slate-900 dark:text-white">{{ $company->name }}
                                        </p>
                            ...
                            <td>
                                <span @class([
                                    'badge gap-1.5',
                                    'badge-success badge-outline' => !$company->closed_at,
                                    'badge-ghost' => $company->closed_at,
                                ])>
                                    <span @class([
                                        'h-1.5 w-1.5 rounded-full',
                                        'bg-success' => !$company->closed_at,
                                        'bg-slate-400' => $company->closed_at,
                                    ])></span>
                                ...
                                <div class="flex justify-end gap-1">
                                        ...
                                        @can('companies.close-fiscal-year')
                                            <a href="{{ route('companies.closing-wizard', $company) }}"
                                                @class([
                                                    ...
                                                    'btn-disabled' => $company->closed_at,
                                                ])>{{ __('Close Fiscal Year') }}</a>
                                        @endcan
                                    @endcannot
                                    @can('companies.destroy')
                                        <form action="{{ route('companies.destroy', $company) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-ghost btn-sm rounded-lg text-error">{{ __('Delete') }}</button>
                                        </form>
                                    @endcan
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="py-20 text-center">
                                <p class="font-semibold text-slate-600 dark:text-slate-300">
                                    {{ __('No companies found.') }}</p>
                                @cannot('access-super-admin-panel')
                                    <p class="mt-1 text-sm text-slate-400">
                                        {{ __('Try changing your filters or create a company.') }}</p>
                                @endcannot
                            </td>
                        </tr>
                    @endforelse
