    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:rounded-3xl dark:border-slate-800 dark:bg-slate-900">
        ...
        <form action="{{ route('management.activity-logs.index') }}" method="GET" class="grid gap-x-4 gap-y-4 p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-5" x-data>
            ...
            <label class="form-control">
                ...
                <input type="search" name="model_identifier" value="{{ $filters['model_identifier'] ?? '' }}" class="input input-bordered w-full" placeholder="{{ __('Number or ID') }}">
            </label>
                                        ...
                                        <div x-cloak x-show="modelOpen" class="space-y-2 sm:hidden p-3">
                                            @foreach ($modelChanges as $change)
                                                <div @class(['rounded-lg border border-slate-200 p-3 text-xs dark:border-slate-800', 'bg-amber-50/70 font-bold dark:bg-amber-950/20' => $change['old'] !== $change['new']])>
                                                    ...
                                                    <dl class="grid gap-2">
                                                        <div @class(['rounded-lg p-2', 'bg-rose-50 dark:bg-rose-950/30' => $change['old'] !== $change['new']])>
                                                            <dt class="text-[11px] font-semibold text-slate-400">{{ __('Previous value') }}</dt>
                                                            <dd>
                                                                <pre @class(['mt-1 whitespace-pre-wrap break-all', 'font-bold text-rose-700 dark:text-rose-300' => $change['old'] !== $change['new']]) dir="ltr">{{ $change['old'] }}</pre>
                                                            </dd>
                                                        </div>
                                                        <div @class(['rounded-lg p-2', 'bg-emerald-50 dark:bg-emerald-950/30' => $change['old'] !== $change['new']])>
                                                            <dt class="text-[11px] font-semibold text-slate-400">{{ __('New value') }}</dt>
                                                            <dd>
                                                                <pre @class(['mt-1 whitespace-pre-wrap break-all', 'font-bold text-emerald-700 dark:text-emerald-300' => $change['old'] !== $change['new']]) dir="ltr">{{ $change['new'] }}</pre>
                                                            </dd>
                                                        </div>
                                                    </dl>
                                                </div>
                                                ...
                                                <tbody>
                                                    @foreach ($modelChanges as $change)
                                                        <tr @class(['bg-amber-50/70 font-bold dark:bg-amber-950/20' => $change['old'] !== $change['new']])>
                                                            ...
                                                            <td>
                                                                <pre @class(['whitespace-pre-wrap break-all text-xs', 'font-bold text-rose-700 dark:text-rose-300' => $change['old'] !== $change['new']]) dir="ltr">{{ $change['old'] }}</pre>
                                                            </td>
                                                            <td>
                                                                <pre @class(['whitespace-pre-wrap break-all text-xs', 'font-bold text-emerald-700 dark:text-emerald-300' => $change['old'] !== $change['new']]) dir="ltr">{{ $change['new'] }}</pre>
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
