                        ...
                        @if ($activity['isRequest'] && collect($activity['requestModels'] ?? [])->isNotEmpty())
                            <div class="flex flex-wrap items-center gap-1.5">
                                @foreach ($activity['requestModels'] as $modelIndex => $model)
                                    ...
                                    @else
                                        <span class="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-semibold text-sky-700 ring-1 ring-inset ring-sky-600/20 dark:bg-sky-950/40 dark:text-sky-300" dir="ltr">{{ $model['modelContextLabel'] }}</span>
                                    @endif
