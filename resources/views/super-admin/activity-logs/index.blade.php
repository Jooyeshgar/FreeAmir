            ...
            <article x-data="{ detailsOpen: false }" class="group rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition duration-200 hover:border-slate-300 hover:shadow-md sm:p-4 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                    <div class="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1">
                        ...
                        @if ($activity['modelContextLabel'] && $activity['modelContextLabel'] !== $activity['contextLabel'])
                            <bdi class="max-w-full truncate font-mono text-xs font-semibold text-sky-700 sm:max-w-64 dark:text-sky-300" dir="ltr">{{ $activity['modelContextLabel'] }}</bdi>
                        @endif
                        @if ($activity['userUrl'])
                ...
                @if ($activity['hasDetails'])
                    <div id="activity-details-{{ $activity['id'] }}" x-cloak x-show="detailsOpen" class="mt-4 border-t border-slate-100 pt-4 dark:border-slate-800">
                        ...
                        @if ($activity['changes']->isNotEmpty())
                            <div @class(['space-y-2 sm:hidden', 'mt-3' => $activity['requestContext']])>
                            ...
                            <div @class(['hidden overflow-x-auto rounded-xl border border-slate-200 sm:block dark:border-slate-800', 'mt-3' => $activity['requestContext']])>
                                <table class="table table-sm min-w-[40rem]">
