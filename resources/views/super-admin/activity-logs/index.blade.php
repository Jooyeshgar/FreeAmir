<x-super-admin-layout :title="__('Activity log')">
    <div class="min-w-0 max-w-full overflow-x-clip">
        <section class="mb-6 px-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end mt-2">
            <div>
            ...
            <div
                class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-3 md:grid md:grid-cols-2 lg:flex">
                <span
                ...
                <a href="{{ route('management.settings') }}"
                    class="btn btn-outline btn-sm w-full sm:w-auto md:w-full lg:w-auto">
                    {{ __('Settings') }}
                </a>
            </div>
        </section>
            ...
            @foreach ($metricCards as $metric)
                <article
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ $metric['label'] }}
                            </p>
                            <strong
                        ...
                        <span
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="1.6" class="h-5 w-5" aria-hidden="true">
        ...
        <section
            ...
            <header
                class="flex flex-col justify-between gap-3 border-b border-slate-200 bg-slate-50/70 px-4 py-4 sm:flex-row sm:items-center sm:px-5 md:flex-col md:items-stretch lg:flex-row lg:items-center dark:border-slate-800 dark:bg-slate-950/30">
                <div>
                ...
                @if ($activeFilterCount > 0)
                    <a href="{{ route('management.activity-logs.index') }}"
                        class="btn btn-ghost btn-sm w-full sm:w-auto md:w-full lg:w-auto">{{ __('Clear filters') }}</a>
                @endif
            </header>

            <form action="{{ route('management.activity-logs.index') }}" method="GET"
                class="grid gap-x-4 gap-y-4 p-4 sm:grid-cols-2 sm:p-5 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-5"
                x-data>
                <label class="form-control sm:col-span-2 md:col-span-1 lg:col-span-2 xl:col-span-1">
                    <span
                ...
                <label class="form-control">
                    ...
                    <select name="user_id" class="select select-bordered w-full">
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @selected((string) ($filters['user_id'] ?? '') === (string) $user->id)>{{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                </label>
                ...
                <label class="form-control sm:col-span-2 md:col-span-1 lg:col-span-2 xl:col-span-1">
                    ...
                    <input type="search" name="model_reference" value="{{ $filters['model_reference'] ?? '' }}"
                        class="input input-bordered w-full" inputmode="numeric"
                        placeholder="{{ __('Model number or ID') }}">
                </label>
                ...
                <div
                    class="grid grid-cols-2 gap-2 sm:col-span-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end md:col-span-1 md:grid md:grid-cols-2 lg:col-span-2 lg:flex xl:col-span-5">
                    <button type="submit" class="btn btn-primary w-full sm:w-auto sm:min-w-36 md:w-full lg:w-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                    ...
                    @if ($activeFilterCount > 0)
                        <a href="{{ route('management.activity-logs.index') }}"
                            class="btn btn-ghost w-full sm:w-auto md:w-full lg:w-auto">{{ __('Reset') }}</a>
                    @endif
                </div>
            </form>
        </section>
        ...
        <section class="space-y-3 sm:space-y-4" aria-label="{{ __('Activity') }}">
            @forelse ($activities as $activity)
                <article x-data="{ detailsOpen: false, requestInputOpen: false, detailsLoaded: false, detailsLoading: false, detailsHtml: '' }"
                    class="group min-w-0 max-w-full overflow-hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition duration-200 hover:border-slate-300 hover:shadow-md sm:p-4 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
                    <div
                        class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4 md:flex-col md:items-stretch lg:flex-row lg:items-center">
                        <div class="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1">
                            @if ($activity['contextUrl'])
                                <a href="{{ $activity['contextUrl'] }}"
                                    class="max-w-full truncate font-mono text-xs font-semibold text-slate-600 hover:underline sm:max-w-80 md:max-w-full lg:max-w-80 dark:text-slate-300"
                                    dir="ltr">{{ $activity['contextLabel'] }}</a>
                            @else
                                <bdi class="max-w-full truncate font-mono text-xs font-semibold text-slate-600 sm:max-w-80 md:max-w-full lg:max-w-80 dark:text-slate-300"
                                    dir="ltr">{{ $activity['contextLabel'] }}</bdi>
                            @endif
                            @foreach ($activity['modelContextLinks']->reverse()->take(5) as $modelContextLink)
                                @if ($modelContextLink['label'] !== $activity['contextLabel'])
                                    @if ($modelContextLink['url'])
                                        <a href="{{ $modelContextLink['url'] }}"
                                            class="max-w-full truncate font-mono text-xs font-semibold text-sky-700 hover:underline sm:max-w-64 md:max-w-full lg:max-w-64 dark:text-sky-300"
                                            dir="ltr">{{ $modelContextLink['label'] }}</a>
                                    @else
                                        <bdi class="max-w-full truncate font-mono text-xs font-semibold text-sky-700 sm:max-w-64 md:max-w-full lg:max-w-64 dark:text-sky-300"
                                            dir="ltr">{{ $modelContextLink['label'] }}</bdi>
                                    @endif
                                @endif
                            @endforeach
                            @if ($activity['modelContextLinks']->count() > 5)
                                <span class="font-mono text-xs font-bold text-slate-500 dark:text-slate-400"
                                    dir="ltr">...</span>
                            @endif
                            @if ($activity['userUrl'])
                                <a href="{{ $activity['userUrl'] }}"
                                    class="min-w-0 break-words font-semibold text-violet-700 transition hover:underline dark:text-violet-400">{{ $activity['userName'] }}</a>
                            @else
                                <span
                                    class="min-w-0 break-words font-semibold text-slate-800 dark:text-slate-100">{{ $activity['userName'] }}</span>
                            @endif
                        </div>
                        <time datetime="{{ $activity['createdAt'] }}"
                            class="self-end text-[11px] text-slate-400 sm:shrink-0 sm:self-auto md:self-end lg:self-auto"
                    ...
                    <div
                        ...
                        @if ($activity['hasDetails'])
                            <button type="button"
                                ...
                                @click="detailsOpen = ! detailsOpen; if (! detailsLoaded && ! detailsLoading) { detailsLoading = true; fetch('{{ route('management.activity-logs.details', $activity['detailsActivityId'] ?? $activity['id']) }}', { headers: { Accept: 'application/json' } }).then(response => response.json()).then(data => { detailsHtml = data.html; detailsLoaded = true; detailsOpen = true; }).finally(() => detailsLoading = false); }"
                    ...
                    @if ($activity['hasDetails'])
                        <div id="activity-details-{{ $activity['id'] }}" x-cloak x-show="detailsOpen"
                            class="border-slate-100 pt-4 dark:border-slate-800">
                            @if ($activity['requestContext'])
                                <dl
                                    class="mb-3 grid gap-2 text-left text-xs sm:grid-cols-2 sm:gap-3 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-5">
                                    @foreach ($activity['requestContext'] as $context)
                                        <div class="rounded-xl bg-slate-50 p-3 text-left dark:bg-slate-950/40">
                                            <dt class="text-right font-semibold text-slate-500" dir="rtl">
                                                {{ $context['label'] }}</dt>
                                            <dd class="mt-1 break-all font-mono text-left" dir="ltr">
                                                {{ $context['value'] }}</dd>
                                        </div>
                                    @endforeach
                                </dl>
            ...
            @empty
                <div
                    ...
                    <span
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="1.6" class="h-7 w-7" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @if ($activities->hasPages())
            <div
                class="mt-6 min-w-0 max-w-full overflow-x-auto rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
