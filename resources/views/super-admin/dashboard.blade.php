<x-super-admin-layout :title="__('Dashboard')">
    ...
    @if ($metrics['unassignedUsers'] > 0)
        <div role="alert"
            ...
            <span
                class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-amber-100 font-bold text-amber-600">!</span>
            <p><b>{{ localizeNumber($metrics['unassignedUsers']) }}</b> {{ __('users have no company assignment.') }}
            </p>
    ...
    <section class="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5" aria-label="{{ __('Platform metrics') }}">
        ...
        <x-kpi-card class="admin-rise" :title="__('Activation rate')" :value="$metrics['activationRate'] . '%'" :unit="__('Company assigned')"
            icon="M4 19V9m5 10V5m5 14v-7m5 7V3" icon-class="bg-amber-50 text-amber-500" />
        <x-kpi-card class="admin-rise" :title="__('Monthly active users')" :value="$metrics['monthlyActiveUsers']" :unit="__('MAU')"
            icon="M3 12h4l3-9 4 18 3-9h4" icon-class="bg-sky-50 text-sky-500" />
        <x-kpi-card class="admin-rise" :title="__('Churn rate')" :value="$metrics['churnRate'] . '%'" :unit="__('Compared with prior period')"
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span @class([
                            ...
                            'bg-emerald-50 text-emerald-600' => $metrics['userGrowthRate'] >= 0,
                            'bg-rose-50 text-rose-600' => $metrics['userGrowthRate'] < 0,
                        ])>MoM {{ localizeNumber(abs($metrics['userGrowthRate'])) }}٪
    ...
    <section class="mt-5 grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,1fr)]">
        <x-management.card class="overflow-hidden">
            <header class="flex flex-wrap items-start justify-between gap-3">
                ...
                <div class="text-end">
                    <b
                        class="block text-2xl text-[#15263b] dark:text-white">{{ localizeNumber($viewModel['activationSteps'][3]['percentage']) }}٪</b>
                ...
                @foreach ($viewModel['activationSteps'] as $step)
                    <div class="relative">
                        <div @class([
                            ...
                            'border-[#16a394] bg-[#16a394] text-white' => $loop->last,
                            'border-slate-100 bg-slate-50 text-[#15263b] dark:border-slate-800 dark:bg-slate-950/40 dark:text-white' => !$loop->last,
                        ])>
                            <div class="flex items-center justify-between"><span
                                    class="text-[10px] font-medium">{{ $step['label'] }}</span>
                            ...
                            <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200/60 dark:bg-slate-700">
                                <div @class([
                                    ...
                                    'bg-white' => $loop->last,
                                    'bg-[#16a394]' => !$loop->last,
                                ]) style="width: {{ min(100, $step['percentage']) }}%">
                                </div>
                            </div>
                        </div>
                        @unless ($loop->last)
            ...
            @if ($metrics['unassignedUsers'] > 0)
                <div
                    class="mt-5 flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50/70 p-4 sm:flex-row sm:items-center">
        ...
        <x-management.card>
            ...
            <div class="mt-8 text-center">
                <b
                    class="text-4xl font-extrabold text-[#15263b] dark:text-white">{{ localizeNumber($metrics['activationRate']) }}٪</b>
        ...
        <x-management.card>
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-3">
                <div class="rounded-xl border border-slate-100 p-4">
                    ...
                    <b
                        class="mt-1 block text-lg">{{ localizeNumber($metrics['monthlyActiveUsers'] > 0 ? round(($metrics['dailyActiveUsers'] / $metrics['monthlyActiveUsers']) * 100, 1) : 0) }}٪</b>
                </div>
                <div class="rounded-xl border border-slate-100 p-4">
                ...
                @foreach ($activityTrend as $day)
                    <div class="text-center">
                        <div class="grid h-16 place-items-center rounded-lg bg-[#16a394] text-[10px] font-bold text-white"
                            style="opacity: {{ max(0.18, $day['count'] / $viewModel['maximumActivity']) }}">
        ...
        <x-management.card class="lg:col-span-2">
            ...
            <div class="mt-5 grid gap-4 sm:grid-cols-3">
                <div class="rounded-xl bg-slate-50 p-4 dark:bg-slate-950/40">
                    <span class="text-[10px] text-slate-400">{{ __('Documents this month') }}</span>
                    <b
                        class="mt-1 block text-2xl">{{ localizeNumber(number_format($metrics['monthlyDocuments'])) }}</b>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 dark:bg-slate-950/40">
                    <span class="text-[10px] text-slate-400">{{ __('Invoices this month') }}</span>
                    <b
                        class="mt-1 block text-2xl">{{ localizeNumber(number_format($metrics['monthlyInvoices'])) }}</b>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 dark:bg-slate-950/40">
            ...
            <div class="mt-6 grid gap-6 border-t border-slate-100 pt-5 sm:grid-cols-2 dark:border-slate-800">
                <div>
                    ...
                    @empty
                        <p
                            class="rounded-lg bg-slate-50 px-3 py-4 text-center text-xs text-slate-400 dark:bg-slate-950/40">
        ...
        <x-management.card variant="dark">
            <header class="flex items-center justify-between">
                ...
                <span class="flex items-center gap-1.5 text-[10px] text-emerald-300">
                    <i class="h-2 w-2 rounded-full bg-emerald-400"></i>{{ __('Stable') }}
                </span>
            </header>
            <div class="mt-5 space-y-3">
                @foreach ([[__('Environment'), __(config('app.env'))], [__('Version'), localizeNumber(config('app.version'))], [__('Registration'), config('app.registration') ? __('Enabled') : __('Disabled')], [__('Email verification'), config('app.email_verification') ? __('Enabled') : __('Disabled')]] as $health)
                    <div class="flex items-center justify-between rounded-xl bg-white/5 p-3">
                        <span class="text-xs text-slate-300">{{ $health[0] }}</span>
    ...
    <section class="mt-5 grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(18rem,1fr)]">
        <x-management.card class="overflow-hidden" :padded="false">
            <header
                ...
                <a href="{{ route('users.index') }}" class="text-xs font-bold text-[#16a394]">{{ __('View all') }}
                    ←</a>
            </header>
            <div class="scrollbar overflow-x-auto">
                <table class="table min-w-[760px]">
                    ...
                    <tbody>
                                ...
                                <td>
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if (auth()->user()->canImpersonateUser($recentUser))
                                            <form action="{{ route('users.impersonate', $recentUser) }}"
                                                method="post">
                                                @csrf
                                                <button
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-10 text-center">
                                    {{ __('No users have been created yet.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="mt-4 grid grid-cols-2 gap-3">
                    @foreach ([[__('Add user'), route('users.create'), 'M12 5v14M5 12h14'], [__('Companies'), route('companies.index'), 'M3 21h18M6 21V7l6-4 6 4v14'], [__('Access control'), route('roles.index'), 'M12 3 4 7v5c0 5 3.4 8 8 9'], [__('Settings'), route('management.settings'), 'M12 15.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Z']] as $action)
                        <a href="{{ $action[1] }}"
                            ...
                            </span><b class="mt-3 block text-xs">{{ $action[0] }}</b></a>
                    @endforeach
                </div>
            </x-management.card>
            <x-management.card>
