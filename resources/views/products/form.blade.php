<div class="grid grid-cols-2 gap-6">
    <div class="col-span-2 md:col-span-1">
        @php
            $hint =
                '<a class="link text-blue-500" href="' . route('product-groups.create') . '">اضافه کردن گروه محصول</a>';
        @endphp
        <x-select title="{{ __('Product group') }}" name="group" id="group" :options="$groups->pluck('name', 'id', 'sstid')" :selected="$product->group ?? null"
            :hint="$hint" />
    </div>
    ...
    <div class="col-span-2 md:col-span-1">
        <x-input name="sstid" id="sstid" title="{{ __('Product SSTID') }}" :value="old(
            'sstid',
    ...
    <div class="col-span-2 md:col-span-1">
        <x-input @input="$event.target.value = $store.utils.formatNumber($event.target.value)" name="selling_price"
            id="selling_price" title="{{ __('Selling price') }}" :value="old('selling_price', $product->selling_price ?? '')"
            placeholder="{{ __('Please insert Selling price') }}" />
    </div>

    <div class="col-span-2 md:col-span-1">
        <x-input name="vat" id="vat" title="{{ __('VAT') }}" :value="old('vat', $product->vat ?? '')" placeholder="0" />
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2">
            <x-input @input="$event.target.value = $store.utils.formatNumber($event.target.value)"
                name="quantity_warning" id="quantity_warning" title="{{ __('Quantity warning') }}" :value="old('quantity_warning', $product->quantity_warning ?? '')"
