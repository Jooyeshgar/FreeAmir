            ...
            <table class="table w-full mt-4">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-2">{{ __('Doc Number') }}</th>
                ...
                <tbody>
                    ...
                    @foreach ($documentFiles as $documentFile)
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <a href="{{ route('documents.show', $documentFile->document->id) }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                                    {{ formatDocumentNumber($documentFile->document->number) }}
                                </a>
                            </td>
                            <td class="px-4 py-2">{{ $documentFile->title }}</td>
                            <td class="px-4 py-2 flex gap-2 items-center">
                                <a href="{{ route('document-files.view', $documentFile) }}" class="btn btn-sm btn-info btn-square" title="{{ __('View') }}">
                                ...
                                <a href="{{ route('document-files.download', $documentFile) }}" class="btn btn-sm btn-info btn-square" title="{{ __('Download') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 10l5 5 5-5M12 15V3" />
                                    </svg>
                                </a>
                                <a href="{{ route('document-files.edit', $documentFile) }}" class="btn btn-sm btn-info btn-square" title="{{ __('Edit') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('document-files.destroy', $documentFile) }}" method="POST" class="m-0" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                                    @method('DELETE')
