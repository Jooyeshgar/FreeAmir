        ...
        <div class="card-body">
            <div class="card-actions">
                <x-button href="{{ route('documents.create') }}"
                    class="btn-primary">{{ __('Create Document') }}</x-button>
            </div>
            <form action="{{ route('documents.index') }}" method="GET">
                <div class="mt-4 mb-4 grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-2 md:col-span-1">
                        <x-text-input data-jdp input_name="date" placeholder="{{ __('date') }}"
                            input_value="{{ request('date') }}" input_class="datePicker"></x-text-input>
                    </div>
                    <div class="col-span-6 md:col-span-3">
                        <x-input name="text" value="{{ request('text') }}"
                            placeholder="{{ __('Search by document title or transaction description') }}" />
                    </div>
            ...
            <table class="table w-full mt-4 overflow-auto">
                ...
                <tbody>
                    @foreach ($documents as $document)
                        <tr>
                            <td class="p-2"><a
                                    href="{{ route('documents.show', $document->id) }}">{{ formatDocumentNumber($document->number) }}</a>
                            </td>
                            ...
                            <td class="p-2">
                                ...
                                <form action="{{ route('documents.destroy', $document) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-sm btn btn-error">{{ __('Delete') }}</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
