<x-card class="rounded-2xl w-full" class_body="p-4">
    <div class="grid grid-cols-1 gap-2 md:grid-cols-2 lg:flex lg:items-end">
        <x-text-input input_name="title" title="{{ __('document name') }}"
            input_value="{{ old('title') ?? $document->title }}" placeholder="{{ __('document name') }}"
            label_text_class="text-gray-500" label_class="w-full lg:flex-1" input_class="w-full lg:max-w-96"></x-text-input>
        ...
        <x-text-input disabled="true" input_value="{{ formatDocumentNumber($previousDocumentNumber) }}" input_name=""
            ...
            label_text_class="text-gray-500 text-nowrap" label_class="w-full lg:flex-1"></x-text-input>
        ...
        <x-text-input data-jdp title="{{ __('date') }}" input_name="date" placeholder="{{ __('date') }}"
            ...
            label_text_class="text-gray-500 text-nowrap" label_class="w-full lg:flex-1"
    ...
    <div class="min-h-96">
        <div id="transactions" x-data="{ activeTab: {{ $total }} }"
            ...
            <template x-for="(transaction, index) in transactions" :key="transaction.id">
                <div :class="{ 'active z-[110]': activeTab === index }" class="transaction relative flex w-max min-w-full gap-2 items-center px-4 pb-3"
                    @click.stop="
                        if (activeTab === index) {
                            activeTab = null;
                            $store.account.clear();
                    ...
                    <div class="flex-none min-w-80 max-w-80"
                        @click.stop="activeTab = index; $store.account.fetch(selectedId)">
                        <x-subject-select :subjects="$subjects" data-subject-select
                            x-init="
                                const root = $el;
            ...
            <button class="flex justify-content gap-4 align-center w-max min-w-full px-4" id="addTransaction"
                ...
                <div class="bg-gray-200 max-h-10 min-h-10 text-sm hover:bg-gray-300 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-slate-600 border-none btn w-full rounded-md btn-active lg:max-h-10 lg:min-h-10 lg:text-base">
...
<div class="mt-4 mb-2 flex gap-2 justify-start sm:justify-end">
    <a href="{{ route('documents.index') }}" type="submit" class="btn btn-sm lg:btn-md btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600"> {{ __('cancel') }}
    </a>
    @if(! $document->exists)
        <button id="submitFormPlus" type="submit" name="submit_action" value="create_new" class="btn btn-sm lg:btn-md btn-default rounded-md dark:bg-slate-700 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-600">
            {{ __('save and create new document') }}
        </button>
    @endif
    <button id="submitForm" type="submit" class="btn btn-sm lg:btn-md text-white btn-primary rounded-md">
