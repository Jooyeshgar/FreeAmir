...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div>
        <x-select name="nationality" id="nationality" title="{{ __('Nationality') }}" :options="$nationalities" :selected="old('nationality', $employee->nationality?->valueName() ?? 'iranian')" required />
    </div>

    <div>
        <x-select name="gender" id="gender" title="{{ __('Gender') }}" :options="$genders" :selected="old('gender', $employee->gender?->valueName() ?? '')" />
    </div>

    <div>
        <x-select name="marital_status" id="marital_status" title="{{ __('Marital Status') }}" :options="$maritalStatuses" :selected="old('marital_status', $employee->marital_status?->valueName() ?? '')" />
    </div>
    ...
    <div>
        <x-select name="duty_status" id="duty_status" title="{{ __('Duty Status') }}" :options="$dutyStatuses" :selected="old('duty_status', $employee->duty_status?->valueName() ?? '')" />
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div>
        <x-select name="work_site_id" id="work_site_id" title="{{ __('Work Site') }}" :title2="'<a href=\'' . route('salary.work-sites.create') . '\' target=\'_blank\' class=\'link link-primary text-xs\'>+ ' . __('New') . '</a>'"
            :options="$workSites->mapWithKeys(fn($ws) => [$ws->id => $ws->name])->toArray()" :selected="old('work_site_id', $employee->work_site_id ?? '')" required />
    </div>

    <div>
        <x-select name="org_chart_id" id="org_chart_id" title="{{ __('Org Chart Position') }}" :title2="'<a href=\'' . route('hr.org-charts.create') . '\' target=\'_blank\' class=\'link link-primary text-xs\'>+ ' . __('New') . '</a>'"
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        <x-select name="insurance_type" id="insurance_type" title="{{ __('Insurance Type') }}" :options="$insuranceTypes" :selected="old('insurance_type', $employee->insurance_type?->valueName() ?? '')" />
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        <x-select name="education_level" id="education_level" title="{{ __('Education Level') }}" :options="$educationLevels" :selected="old('education_level', $employee->education_level?->valueName() ?? '')" />
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div>
        <x-select name="employment_type" id="employment_type" title="{{ __('Employment Type') }}" :options="$employmentTypes" :selected="old('employment_type', $employee->employment_type?->valueName() ?? '')" />
    </div>
