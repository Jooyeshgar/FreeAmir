        ...
        <div class="card-body" x-data="{
            ...
            <div class="card-actions">
                ...
                @if($currentParent)
                    ...
                    <span class="ml-2 lg:text-lg leading-[3rem] font-semibold grow">{{ $currentParent->name }}</span>
                    <a href="{{ $upUrl }}" class="btn btn-sm btn-outline">
                        {{ __('Back') }}
                    </a>
                    @if(! $currentParent->children()->exists())
                        <button class="btn btn-sm btn-primary" x-on:click="openTransferModal('{{ $currentParent->id }}', '{{ $currentParent->name }}')" title="{{ __('Transfer transactions from this subject and its descendants to another') }}">
                ...
                <div class="modal-box w-11/12 max-w-2xl overflow-visible relative">
                    ...
                    <form action="{{ route('subjects.transfer') }}" method="POST" x-data="{ createNewSubject: true }">
                        ...
                        <div class="modal-action">
                            <button type="button" onclick="document.getElementById('transferModal').close()" class="btn btn-sm">{{ __('Cancel') }}</button>
                            <button type="submit" class="btn btn-sm btn-primary">{{ __('Transfer Transactions') }}</button>
                        </div>
                    </form>
                </div>
            </dialog>
