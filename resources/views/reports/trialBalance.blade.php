        ...
        <div class="card-body">
            ...
            <form action="{{ route('reports.trial-balance') }}" method="get">
                ...
                <div class="grid grid-cols-12 gap-3 items-end">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        <x-input name="start_document_number" value="{{ $start_document_number }}" class="w-full" placeholder="{{ __('Document start number') }}"></x-input>
                    </div>
                    <div class="col-span-6 sm:col-span-3">
                    ...
                    <div class="col-span-12 sm:col-span-3 flex flex-col gap-2">
                        <label class="flex items-center gap-2 text-sm text-gray-700">
                            ...
                            <span>{{ __('Include children (2 levels)') }}</span>
                        </label>
                        <label class="flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="two_columns" value="1" class="checkbox checkbox-sm" {{ $columns_number === 2 ? 'checked' : '' }}>
                            <span>{{ __('Two column view (balance only)') }}</span>
                        </label>
                    </div>
            ...
            <div class="mt-4 border border-gray-300 rounded-md overflow-auto" style="max-height: 70vh;">
                <table class="table table-zebra w-full">
                    ...
                    <tbody id="tb-rows">
                        @forelse ($subjects as $index => $subject)
                            ...
                            <tr class="hover" data-lazy-row data-index="{{ $index }}">
                                ...
                                @if ($columns_number === 4)
                                    ...
                                    <td class="px-4 py-2 border-r border-gray-400">{{ formatNumber($subject->credit) }}</td>
                                @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $columns_number === 4 ? 6 : 4 }}" class="text-center py-4 text-gray-500">{{ __('No data found for the selected filters.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                <div id="tb-sentinel" class="h-8"></div>
            </div>
            <script>
                (() => {
