    <div class="grid grid-cols-1 md:grid-cols-4 gap-4"
        ...
        <div>
            <x-select name="thursday_status" id="thursday_status" :title="__('Thursday Status')" :options="$thursdayStatusOptions" :selected="old('thursday_status', $workShift->thursday_status->valueName() ?? 'half_day')" required x-data="{}"
                x-on:change="$dispatch('thursday-status-changed', { value: $event.target.value })" />
        </div>
        <div x-data="{ show: '{{ old('thursday_status', $workShift->thursday_status->valueName() ?? 'half_day') }}' === 'half_day' }" x-on:thursday-status-changed.window="show = $event.detail.value === 'half_day'" x-show="show">
