                ...
                datasets: [
                    {
                        label: "رئال",
                        data: [
                            15250000, 11890000, 33754000, 21507000, 31708000,
                        ], // مقادیر داده‌ها
...
if (
    document
        .querySelector(".selfSelectBoxContainer")
        .closest("#originalTransactions")
) {
    ...
    codeInput.addEventListener("input", () => {
        ...
        let matchedSpan = Array.from(spans).find(
            (span) => span.getAttribute("data-code") === code
        );
    ...
    function formatCode(e) {
        ...
        return (
            (e = t.join("/")),
            ["fa", "fa_IR"].includes("fa") && (e = convertToFarsi(e)),
            e
        );
    }
    ...
    function searchQuery(e, t) {
        fetch("/subjects/search", {
            ...
            .then((e) => {
                (resultDivs[t].style.display = "none"),
                    (searchResultDivs[t].style.display = "block"),
                    0 == e.length
                        ? (searchResultDivs[t].innerHTML =
                              '<span class="block text-center">چیزی پیدا نشد!</span>')
                        : (searchResultDivs[t].innerHTML = ""),
                    e.forEach((e, i) => {
                        let n = e.name,
                            a = e.code,
                        ...
                        `;
                            (searchResultDivs[t].innerHTML += r),
                                l.forEach((e) => {
                                    ...
                                    <a href="javascript:void(0)"
                                        ...
                                        <span class="codeList" data-name="${
                                            e.name
                                        }" data-code="${e.code}" hidden></span>
                                        <span class="selfItemCode" data-en-code="${
                                            e.code
                                        }">
                                            ${formatCode(e.code)}
                                        </span>
                                        <span class="selfItemId hidden">${
                                            e.id
                                        }</span>
                                    </a>
                                    `;
    ...
    const debouncedSearch = debounce(function (e, t) {
        ...
        n
            ? searchQuery(n, t)
            : ((resultDivs[t].style.display = "block"),
