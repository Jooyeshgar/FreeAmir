...
جدول `audit_logs` اطلاعات عملیاتی تغییرات احراز هویت‌شده را نگهداری می‌کند. سرویس
`AuditLogger` رویدادهای ایجاد، ویرایش و حذف مدل‌های Eloquent را ثبت می‌کند و فیلدهای حساس
مانند رمز عبور، توکن و کلید خصوصی را پیش از ذخیره حذف می‌کند. رکوردهای مدل `AuditLog` از
طریق Eloquent قابل ویرایش یا حذف نیستند و مدیر کل می‌تواند آن‌ها را در مسیر
`/admin/audit-logs` جستجو و فیلتر کند.
