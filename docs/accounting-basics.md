...
**هشدار مهم**: برای تولید کد سرفصل‌ها حتما از `generateCode()` در مدل Subject استفاده کنید:
...
// سرفصل جدید: اشخاص متفرقه/محمدی با کد مثلا 012001001
```
...
// ابتدا سرفصل والد را پیدا کنید (مثلا صندوق اصلی)
...
سال مالی در امیر به عنوان "شرکت" (Company) شناخته می‌شود. هر شرکت نمایانگر یک سال مالی مستقل است و مقدار عددی سال (مثلا 1403) در ستون `fiscal_year` همان رکورد نگهداری می‌شود.
...
class Company extends Model {    // هر ردیف نمایانگر یک سال مالی است
    protected $fillable = [
        ...
        'fiscal_year',     // سال مالی به صورت عدد صحیح (مثلا 1403)
    ];
}
```
